...
# Picard (Java command line tools for manipulating HTS data and formats)
...
#cut (cut sections from lines of a file) -f4,5,9 (fields 4, 5, 9 from each line) <GtfFile | sed (find 's/gene_id //g' in input) | gawk (pattern scanning) '{print...}' | tr (translating / deleting characters) ... -t (truncates set1) > <OUTPUT>
